...
Codex, Claude Code, pi, and omp native names take precedence. Other stores fall back to the first useful user prompt. Generic startup titles and injected context are ignored.
...
Claude Code, Codex, Droid, OpenCode, Cursor, GitHub Copilot, Grok, Kimi, Gemini, pi, and omp (oh-my-pi, including `$XDG_DATA_HOME/omp`). omp subagent transcripts are not listed. Missing stores are ignored.
